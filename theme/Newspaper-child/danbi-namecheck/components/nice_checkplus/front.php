	...
	function wpmem_register_form_rows($rows, $toggle) {
		if ($toggle == 'new') {
			...
			if ($this->plugin->get_option('nice_ckeckplus_fields','birthdate')) {
				$new_rows[] = $this->display_register_input('birthdate','생년월일');
			}
			if ($this->plugin->get_option('nice_ckeckplus_fields','gender')) {
				$new_rows[] = $this->display_register_input('gender','성별');
			}
			if ($this->plugin->get_option('nice_ckeckplus_fields','national')) {
				$new_rows[] = $this->display_register_input('national','국적 구분');
			}
